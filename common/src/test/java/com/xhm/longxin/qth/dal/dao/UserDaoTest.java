...
public class UserDaoTest extends BaseDaoTest {
	...
	public void setUp() {
		...
		db.table("qth_user").clean().insert(6, new DataMap() {
			{
				this.put("login_id", DataGenerator.repeat("zhangren", "xhm",
						"admin", "zhangren1", "xhm1", "admin1"));
				this.put("name", DataGenerator.repeat("姓名1", "姓名2", "姓名3",
						"姓名4", "姓名5", "姓名6"));
				this.put("gender", Gender.FEMALE);
				this.put("email", "abc@abc.com");
				this.put("phone_area", "0571");
				this.put("phone_number", "85022088");
				this.put("company", DataGenerator.repeat("公司1", "公司2", "公司3",
						"公司4", "公司5", "公司6"));
		...
		db.table("qth_pro_category").clean().insert(3, new DataMap() {
			{
				...
				this.put("name", DataGenerator
						.repeat("测试类别1", "测试类别2", "测试类别3"));
		...
		db.table("qth_user_interest").clean().insert(7, new DataMap() {
			{
				this.put("login_id", DataGenerator.repeat("zhangren",
						"zhangren", "zhangren", "xhm1", "xhm1", "xhm1", "xhm"));
				this
						.put("interest", DataGenerator.repeat("buy", "buy",
								"sale"));
	...
	public void testUserUpdate() {
		//zhangren对类别1，2有买兴趣，对3有卖兴趣，重点测试对这个类别信息的编辑
		User userToBeUpdated = userDao.getUserByLoginId("zhangren");
		want.number(userToBeUpdated.getBuyInterests().size()).eq(2);
		want.number(userToBeUpdated.getSaleInterests().size()).eq(1);
		//改成其中一个类别由买兴趣变成卖兴趣，另1个类别由卖兴趣变买兴趣，验证之
		UserInterest toBeDeleted=userToBeUpdated.getBuyInterests().get(0);
		userToBeUpdated.getBuyInterests().remove(0);//减少一个买兴趣类别
		UserInterest i = new UserInterest();
		i.setInterest(UserInterestType.SALE);
		i.setValue(toBeDeleted.getValue());
		i.setLoginId(toBeDeleted.getLoginId());
		userToBeUpdated.getSaleInterests().add(i);//对应地增加为卖兴趣
		userDao.updateUser(userToBeUpdated);
		//验证
		User userUpdated=userDao.getUserByLoginId("zhangren");
		want.number(userUpdated.getBuyInterests().size()).eq(1);
		want.number(userUpdated.getSaleInterests().size()).eq(2);
